...
export function AdminMedia({ initialState }) {
  ...
  return <main className="admin-shell"><section className="media-studio">
    <header className="media-studio-header"><div><p className="eyebrow">Publishing studio</p><h1>Article media</h1><p>Prepare a visual, then publish it with the article context intact.</p></div><div className="media-header-actions"><Link className="admin-secondary" href="/admin/">Dashboard</Link><Link className="admin-secondary" href="/admin/articles/">Articles</Link><span className="media-status">Git-backed</span><button className="admin-secondary" onClick={logout}>Sign out</button></div></header>
  ...
  {editorFile && <FilerobotEditor file={editorFile} cover={setAsCover} onSave={(nextFile) => { setFile(nextFile); setEditorFile(null); setMessage("Image edits are ready to publish."); }} onClose={() => setEditorFile(null)} />}</section></main>;
}
