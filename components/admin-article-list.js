...
export function AdminArticleList({ articles }) {
  ...
  const published = articles.map((article) => {
    ...
    return { ...article, ...(draft?.article || {}), draftKey, browserDraft, isDraft: article.localDraft || browserDraft, href: `/admin/edit/${article.slug}/` };
  });
