...
import { Footer, Header, Newsletter } from "./site";
...
export function StaticPage({ page }) {
  ...
  return <><Header /><main className="page"><h1>{content.title}</h1><article className={`markdown ${page}`}><Markdown>{content.content}</Markdown></article><Newsletter /></main><Footer /></>;
}
