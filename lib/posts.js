...
function formatPost(fileName) {
  ...
  return { title: data.title, description: data.description, author: data.author, image: data.image || null, imageAlt: data.imageAlt || data.title, featured: data.featured === true, date, updated, tags: data.tags || [], slug, path: `/${year}/${month}/${day}/${slug}/`, content: body };
}
