...
export function publicationTimestamp(previous, article, now = new Date().toISOString()) {
  if (previous?.publishedAt) return previous.publishedAt;
  if (article.status !== "published") return null;
  return !previous || previous.status === "draft" ? now : null;
}
...
export async function saveArticle(input) {
  ...
  const previous = existing ? articleFromSource(existingSlug, decode((await getBlob(existing.sha)).content)) : null;
  article.publishedAt = publicationTimestamp(previous, article);
...
export const mediaRules = { maxImageBytes, updateCover, validateImagePath, articleFromSource, articleSource, publicationTimestamp, mergeLocalArticles, clearFeatured, mediaUploadPath, mediaFromTree, replaceMediaReferences, sortMediaByAddedAt };
