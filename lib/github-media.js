...
async function postsFromTree(tree) {
  ...
  return Promise.all(files.map(async (entry) => {
    ...
    return { slug, path: entry.path, title: parsed.data.title, description: parsed.data.description || "", date, status: parsed.data.status === "draft" ? "draft" : "published", image: parsed.data.image || "", imageAlt: parsed.data.imageAlt || parsed.data.title, featured: parsed.data.featured === true };
  }));
}
