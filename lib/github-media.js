...
async function postsFromTree(tree) {
  ...
  return Promise.all(files.map(async (entry) => {
    ...
    return { slug, path: entry.path, title: parsed.data.title, description: parsed.data.description || "", date, image: parsed.data.image || "", imageAlt: parsed.data.imageAlt || parsed.data.title, featured: parsed.data.featured === true };
  }));
}
...
function articleFromSource(slug, source) {
  ...
  const date = parsed.data.date instanceof Date ? parsed.data.date.toISOString().slice(0, 10) : String(parsed.data.date || "");
  return { slug, title: parsed.data.title || "", description: parsed.data.description || "", author: parsed.data.author || "", date, tags: Array.isArray(parsed.data.tags) ? parsed.data.tags : [], image: parsed.data.image || "", imageAlt: parsed.data.imageAlt || "", featured: parsed.data.featured === true, content: parsed.content.trim() };
}
...
export const mediaRules = { maxImageBytes, updateCover, validateImagePath, articleFromSource, articleSource, mergeLocalArticles, clearFeatured, mediaUploadPath };
