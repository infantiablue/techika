...
export function optimizedOutputType(fileType) {
  return fileType === "image/png" ? "image/png" : "image/jpeg";
}

function optimizedName(name, type) {
  return `${name.replace(/\.[^.]+$/, "") || "image"}.${type === "image/png" ? "png" : "jpg"}`;
}

function canvasBlob(canvas, type) {
  return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("The browser could not optimize this image.")), type, quality));
}
...
export async function optimizeImage(file, { cover = false } = {}) {
  ...
  try {
    ...
    const outputType = optimizedOutputType(file.type);
    if (outputType === "image/jpeg") { context.fillStyle = "#fff"; context.fillRect(0, 0, dimensions.width, dimensions.height); }
    context.drawImage(image, 0, 0, dimensions.width, dimensions.height);
    const blob = await canvasBlob(canvas, outputType);
    const optimized = shouldUseOptimized(file.size, blob.size);
    const nextFile = optimized ? new File([blob], optimizedName(file.name, outputType), { type: blob.type }) : file;
...
export const imageOptimizationRules = { bodyBounds, maxImageBytes, minimumSaving, quality };
